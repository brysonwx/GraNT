...
from x_models.backbones import GnnNets
        ...
        if batch.x.shape[0] == 1 or batch.batch[-1] == 0:
            ...
            if model_args.enable_prot is True:   
                if model_args.single_target is True:
                    ...
                    prototypes_of_correct_class = torch.t(model.model.get_prototype_class_identity().to(model_args.device)[:, batch.y.squeeze().long()].bool()).to(model_args.device)
                else:
                    correct_class_indices = torch.argmax(virtual_label, dim=1)
                    prototypes_of_correct_class = torch.t(model.model.get_prototype_class_identity().to(model_args.device)[:, correct_class_indices.squeeze().long()].bool()).to(model_args.device)
...
def pipeline(args):
    ...
    # print('Best validation score: {}'.format(valid_curve[best_val_epoch]))
    logger.info('Best validation score: {}'.format(valid_curve[best_val_epoch]))
    logger.info('Test score: {}'.format(test_curve[best_val_epoch]))
